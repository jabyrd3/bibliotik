...
const setup = () => {
    let queue = [];
    rpcPass = keytar.getPassword('biblio', `bbtk_${config.remoteSettings.userName}`);
    bibPass = keytar.getPassword('biblio', `bbtk_${config.bibSettings.userName}`);
    [{
    ...
    if (queue.length > 0) {
        inquirer.prompt(queue.map(item => {
            ...
            keytar.addPassword('biblio', `bbtk_${config.remoteSettings.userName}`, res.rpcpw);
            keytar.addPassword('biblio', `bbtk_${config.bibSettings.userName}`, res.bbpw);
        }).catch(err => {
            console.log(`err`, err);
        });
    }
...
const downloadLink = (link) => {
    ...
    https.get({
        ...
        res.pipe(handle)
        spawn('scp', [config.localSettings.tempName, `${config.remoteSettings.sshName}:${config.remoteSettings.sshPath}`])
            .on('close', () => {
...
const addTorrent = () => {
    ...
    var child = exec(`transmission-remote ${config.remoteSettings.rpcUrl} --auth=${config.remoteSettings.userName}:${rpcPass} -a ${__dirname}/${config.localSettings.tempName}`)
    child.on('close', code => {
        pollTransmission(getTorrentId(), 1000);
    });
};

const doneTorrent = (meta, data) => {
...
const pollTransmission = (metadata, interval) => {
    // poll transmission for torrent copmletion every {interval} seconds, recursive until met
    let list = exec(`transmission-remote ${config.remoteSettings.rpcUrl} --auth=${config.remoteSettings.userName}:${rpcPass} -l`);
    list.stdout.on('data', data => {
        return doneTorrent(metadata, data) ? torrentDone(metadata) : setTimeout(pollTransmission.bind(this, metadata, interval), interval);
    });
};
...
const query = () => {
    nightmare
        ...
        .click('.submit')
        .wait('#search_header')
        .goto(`https://bibliotik.me/torrents/?search=${process.argv[2].replace(/"/g, '')}`)
        .wait('#torrents_table')
        .evaluate(() => {
        ...
        .then(result => {
            ...
            nightmare.cookies.get().then(ck => { cookies = ck });
            ...
            $('tbody tr').each((ind, item) => {
                var target = $(item);
                options.push({
                    name: `${target.find('a').html()} ${target.find('.torFormat').text()}`,
